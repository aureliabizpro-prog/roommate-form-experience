	    ...
	    return (
	        <main className="flex flex-col min-h-screen w-full bg-slate-50 pt-6 sm:pt-8">
	            <div className="flex-grow flex flex-col items-center justify-start px-4 pb-6 overflow-y-auto">
	                <div className="w-full max-w-sm sm:max-w-md md:max-w-lg mx-auto space-y-4">
	                    ...
	                    <div className="w-full">
	                        <div className="flex justify-between items-center mb-2">
	                            <p className="text-xs sm:text-sm font-semibold text-orange-600 leading-tight">{progressText}</p>
	                            <p className="text-xs sm:text-sm font-bold text-gray-800 bg-white px-3 py-1.5 rounded-full shadow-sm border border-orange-100">
                        ...
                        >
                            {!isComplete ? (
                                ...
                                <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6 sm:p-8 text-center">
                                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-3">🙌 您已完成所有問題！</h2>
                                    <p className="text-sm sm:text-base text-gray-600 mb-6">準備好看看您的專屬統計，並開始媒合了嗎？</p>
                                    <motion.button
                                        whileHover={{ scale: 1.05 }}
                                        whileTap={{ scale: 0.95 }}
                                        onClick={handleSubmit}
                                        className="w-full sm:w-auto px-8 py-3 bg-green-500 text-white font-semibold text-base rounded-full shadow-md hover:bg-green-600 active:bg-green-700 transition-colors"
                                    >
                                        完成並提交
                                    </motion.button>
                                </div>
                    ...
                    {/* Navigation Buttons - Right below card */}
                    <div className="w-full flex justify-between items-center gap-3 pt-2">
                        <motion.button
                            onClick={handleBack}
                            disabled={currentQuestion === 0}
                            className="flex-1 min-h-[48px] px-5 py-3 bg-slate-200 text-gray-800 font-medium rounded-full disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-300 active:bg-slate-400 transition-colors shadow-sm"
                            whileTap={{ scale: 0.97 }}
                        >
                            上一步
                        </motion.button>
                        {currentQuestion < totalQuestions && (
                            <motion.button
                                onClick={handleNext}
                                className="flex-1 min-h-[48px] px-5 py-3 bg-orange-300 text-gray-800 font-semibold rounded-full hover:bg-orange-400 active:bg-orange-500 transition-colors shadow-sm"
                                whileTap={{ scale: 0.97 }}
                            >
                                下一步
                            </motion.button>
