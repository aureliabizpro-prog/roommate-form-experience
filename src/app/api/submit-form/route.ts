...
export async function POST(req: NextRequest) {
    try {
        ...
        // Log the full URL being sent to Google Forms for debugging
        console.log("Submitting to Google Form URL:", googleFormUrl.toString());

        await fetch(googleFormUrl.toString(), {
            method: 'POST',
            // mode: 'no-cors', // Removed mode: 'no-cors' for server-side fetch
        ...
        console.error('Submit form failed:', error); // Changed message for clarity
        return NextResponse.json(
            { message: 'Form submission failed on server', error: (error as Error).message }, // Include error message
            { status: 500 } // Explicitly return 500 if an unexpected error occurs
        );
    }
}
"q4_cleaning_habits": "entry.532463167",
